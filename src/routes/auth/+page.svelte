<script lang="ts">
	 ...
	 */
	let redirect_url = data.callback + "?success=true&token=" + encodeURIComponent(data.access_token as string) 
</script>
	...
	>
		<section class="rounded-md p-4 w-full border">
			...
			<div class="flex items-center justify-center mt-4">
				...
				{:else}
					<a
						href="/join?callback={data.callback}&access_token={data.access_token}"
					>
						<button
